...
subt(A,B)->
    gen_server:call(?MODULE, {subt,A,B},infinity).
divi(A,B)->
...
handle_call({add,A,B}, _From, State) ->
    Reply=rpc:call(node(),calc_lib,add,[A,B]),
    {reply, Reply, State};
handle_call({subt,A,B}, _From, State) ->
    Reply=rpc:call(node(),calc_lib,subt,[A,B]),
    {reply, Reply, State};
handle_call({divi,A,B}, _From, State) ->
    Reply=rpc:call(node(),calc_lib,divi,[A,B]),
    {reply, Reply, State};
